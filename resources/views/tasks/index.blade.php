            ...
            <tbody>
                @forelse ($tasks as $task)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4">
                            {{ Str::limit($task->body, 30) }}
                        </td>
                        <td class="px-6 py-4">
                            <x-badge :status="$task->status->name">
                                {{ $task->status->name }}
                            </x-badge>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 inline-flex gap-4">
                            ...
                            <form action="{{ route('tasks.destroy', $task) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('¿Seguro que deseas borrar esta tarea?')"><svg xmlns="http://www.w3.org/2000/svg" fill="none"
